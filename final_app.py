        ...
        def run_motor(self):
            ...
            while True:
                ...
                if (self.on):
                    ...
                    if (self.mode == 1):
                        if 0 in START_MOTOR:
                            if(np.abs(self.hip_amp*self.left_hip_trajectory(self.w, delta_time, 0,1))<0.5*(np.pi/180)):
                                START_MOTOR[2] = 1 
                            if(np.abs(self.knee_amp*self.left_knee_trajectory(self.w, delta_time, 0,1))<7*(np.pi/180) and np.abs(self.knee_amp*self.left_knee_trajectory(self.w, delta_time, 0,1))>5*(np.pi/180)):
                                START_MOTOR[3] = 1
                            if(np.abs(self.hip_amp*self.right_hip_trajectory(self.w, delta_time + self.right_delay, 0,1))<0.5*(np.pi/180)):
                                START_MOTOR[0] = 1
                                print(delta_time)
                            if(np.abs(self.knee_amp*self.right_knee_trajectory(self.w, delta_time + self.right_delay, 0,1))<7*(np.pi/180) and np.abs(self.knee_amp*self.right_knee_trajectory(self.w, delta_time + self.right_delay, 0, 1))>5*(np.pi/180)):
                                START_MOTOR[1] = 1
                        # START_MOTOR=[1,1,1,1]
                        if(delta_time < 20):
                            if (self.left_hip and START_MOTOR[2] == 1):
                                    self.left_hip_motor.attain(
                                        self.hip_amp*self.left_hip_trajectory(self.w, delta_time, 0, 1)+self.hip_offset, self.hip_amp*self.left_hip_trajectory_vel(self.w, delta_time, 0, 1), 0, self.hip_kp*delta_time/10, 1)
                            if (self.left_knee and START_MOTOR[3] == 1):
                                    self.left_knee_motor.attain(
                                        self.knee_amp*self.left_knee_trajectory(self.w, delta_time, 0, 1)+self.knee_offset+ 6*np.pi/180, self.knee_amp*self.left_knee_trajectory_vel(self.w, delta_time, 0, 1), 0, self.knee_kp*delta_time/10, 1)
                            if (self.right_hip and START_MOTOR[0] == 1):
                                    self.right_hip_motor.attain(
                                                -self.hip_amp*self.right_hip_trajectory(self.w, delta_time + self.right_delay, 0, 1)+self.hip_offset, -self.hip_amp*self.right_hip_trajectory_vel(self.w, delta_time + self.right_delay, 0, 1), 0, self.hip_kp*delta_time/10, 1)
                            ...
                            if (self.right_knee and START_MOTOR[1] == 1):
                                    self.right_knee_motor.attain(
                                        -self.knee_amp*self.right_knee_trajectory(self.w, delta_time + self.right_delay, 0, 1)-self.knee_offset + 3*np.pi/180, -self.knee_amp*self.right_knee_trajectory_vel(self.w, delta_time + self.right_delay, 0, 1), 0, self.knee_kp*delta_time/10, 1)
                            ...
                            if (self.right_hip and START_MOTOR[0] == 1):
                                    self.right_hip_motor.attain(
                                                -self.hip_amp*self.right_hip_trajectory(self.w, delta_time + self.right_delay, 0, 1), -self.hip_amp*self.right_hip_trajectory_vel(self.w, delta_time + self.right_delay, 0, 1), 0, self.hip_kp, self.kd)
                                                # self.hip_amp*self.right_hip_trajectory(self.w, delta_time + self.right_delay, 0, 1)-self.hip_offset, self.hip_amp*self.right_hip_trajectory_vel(self.w, delta_time + self.right_delay, 0, 1), 0, self.hip_kp, self.kd)
                            if (self.right_knee and START_MOTOR[1] == 1):
                                    self.right_knee_motor.attain(
                                        -self.knee_amp*self.right_knee_trajectory(self.w, delta_time + self.right_delay, 0, 1)-self.knee_offset, -self.knee_amp*self.right_knee_trajectory_vel(self.w, delta_time + self.right_delay, 0, 1), 0, self.knee_kp, self.kd)
